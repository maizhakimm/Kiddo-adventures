...
function AuthModal({onClose,onLogin,refCode}){
  ...
  async function submit(e){e.preventDefault();setBusy(true);setMsg('');try{
    const d=await api(mode==='login'?'/api/login':'/api/signup',{method:'POST',body:JSON.stringify(form)});
    localStorage.setItem('kiddo_parent',JSON.stringify(d));onLogin(d);
  ...
  return <Modal title={mode==='login'?'Log Masuk Ibu Bapa':'Daftar Akaun Ibu Bapa'} onClose={onClose}>
    ...
    <form className="form" onSubmit={submit}>
      ...
      {msg&&<div className="alert">{msg}</div>}<button className="primary" disabled={busy}>{busy?'Tunggu…':mode==='login'?'Masuk':'Cipta Akaun'}</button>
    </form>
  </Modal>;
}
...
function ParentGate({parent,onClose,onVerified}){
  ...
  async function verify(e){e.preventDefault();setBusy(true);setMsg('');try{
    await api('/api/login',{method:'POST',body:JSON.stringify({email:parent.email,password})});onVerified();
  }catch{setMsg('Password ibu bapa tidak betul.')}finally{setBusy(false)}}
  return <Modal title="Kawasan Ibu Bapa 🔐" onClose={onClose}>
    <p className="parentGateText">Masukkan semula password ibu bapa untuk mengubah atau memadam profil anak.</p>
    <form className="form" onSubmit={verify}><input autoFocus type="password" required minLength="6" placeholder="Password ibu bapa" value={password} onChange={e=>setPassword(e.target.value)}/>{msg&&<div className="alert">{msg}</div>}<button className="primary" disabled={busy}>{busy?'Semak…':'Buka Tetapan Profil'}</button></form>
  </Modal>;
}
...
function ProfileEditor({parent,profile,onClose,onSaved}){
  ...
  const[msg,setMsg]=useState('');const[busy,setBusy]=useState(false);const[confirmDelete,setConfirmDelete]=useState(false);
  async function save(e){e.preventDefault();setBusy(true);setMsg('');try{
    ...
    if(isEdit)await api(`/api/child-profiles/${profile.id}`,{method:'PUT',body:JSON.stringify(body)});else await api('/api/child-profiles',{method:'POST',body:JSON.stringify(body)});
  ...
  return <Modal title={isEdit?'Urus Profil Anak':'Tambah Profil Anak'} onClose={onClose}>
    <form className="form" onSubmit={save}>
      <input required placeholder="Nama anak" value={form.name} onChange={e=>setForm({...form,name:e.target.value})}/>
      <select required value={form.age} onChange={e=>setForm({...form,age:e.target.value})}><option value="">Pilih umur</option>{[3,4,5,6,7].map(age=><option key={age} value={age}>{age} tahun</option>)}</select>
      <div className="avatarLabel">Pilih avatar</div><div className="avatars">{AVATARS.map(a=><button type="button" key={a.id} className={`avatarBtn ${form.avatar===a.id?'active':''}`} onClick={()=>setForm({...form,avatar:a.id})}>{a.icon}</button>)}</div>
      {msg&&<div className="alert">{msg}</div>}<button className="primary" disabled={busy}>{busy?'Tunggu…':isEdit?'Simpan Perubahan':'Tambah Profil'}</button>
      {isEdit&&!confirmDelete&&<button type="button" className="dangerBtn" onClick={()=>setConfirmDelete(true)}>🗑️ Padam Profil</button>}
      {isEdit&&confirmDelete&&<div className="deleteConfirm"><b>Padam {profile.name}?</b><p>Semua progress permainan profil ini akan dipadam dan tidak boleh dipulihkan.</p><div><button type="button" className="ghost" onClick={()=>setConfirmDelete(false)}>Batal</button><button type="button" className="dangerBtn" disabled={busy} onClick={remove}>Ya, Padam</button></div></div>}
    </form>
  </Modal>;
}
...
function makeChallenge(gameKey,level){
  ...
  if(gameKey==='huruf'){
    ...
    return{prompt:`Cari huruf ${answer}`,visual:'🔤',options,answer};
  }
  if(gameKey==='nombor'){
...
function GameScreen({game,child,level,onBack,onComplete}){
  ...
  async function choose(value){if(result==='correct'||busy)return;const ok=String(value)===String(challenge.answer);if(!ok){setResult('wrong');setTimeout(()=>setResult(''),700);return}setResult('correct');setBusy(true);try{
    await api('/api/progress',{method:'POST',body:JSON.stringify({child_id:child.id,game_key:game.game_key,level_reached:Math.min(level+1,game.total_levels),stars:level})});
  ...
  return <section className={`gameScreen level-${worldMeta[game.game_key]?.tone||'sky'}`}>
    ...
    <div className={`gameBoard ${result}`}><div className="gameVisual">{challenge.visual}</div><h1>{challenge.prompt}</h1><div className="answerGrid">{challenge.options.map((o,i)=><button key={`${o}-${i}`} onClick={()=>choose(o)}>{o}</button>)}</div>{result==='wrong'&&<div className="gameFeedback bad">Cuba lagi 💪</div>}{result==='correct'&&<div className="gameFeedback good">Bagus! ⭐<button onClick={()=>onComplete(level+1)}>Level seterusnya →</button></div>}</div>
  </section>;
}
...
function LevelPage({game,child,onBack,onPlay,refreshKey}){
  ...
  return <section className={`levelPage level-${meta.tone||'sky'}`}><div className="levelTopbar"><button className="levelBack" onClick={onBack}>← Dunia</button><div className="levelTitle"><span className="levelEmoji">{meta.icon||'🎮'}</span><div><span className="eyebrow">{meta.world||game.name_ms}</span><h1>{game.name_ms} • {child.name}</h1><p>{meta.tag||game.name_en}</p></div></div><div className="levelScene">{meta.scene}</div></div>
    <div className="levelMapWrap"><div className="levelMapLabel"><b>Pilih level</b><span>Ikut laluan sampai habis ✨</span></div><div className="levelTrack">{missions.map(n=>{const open=n<=reached;return <div className={`levelNodeWrap ${open?'open':'locked'}`} key={n}><button className="levelNode" disabled={!open} onClick={()=>open&&onPlay(n)}><span>{open?'▶':'🔒'}</span><b>{n}</b></button></div>})}</div></div>
  </section>;
}
...
function ParentDashboard({parent,games,onLogout}){
  ...
  useEffect(()=>{load()},[]);
  function openParentSettings(){setGate(true)}
  function verified(){setGate(false);setEditor(selected)}
